...
func _on_http_request_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	if response_code == 200:
		...
		if error == OK:
			...
			if FileAccess.file_exists("res://dialogues/coversations/sushil.dialogue"):
				DirAccess.remove_absolute("res://dialogues/coversations/sushil.dialogue")
			
			# Create dialogue file content
			var dialogue_file_content = "~ start\n"
			
			# Add dialogues from JSON
			for dialogue in dialogue_data.ex_1.npc_1:
				dialogue_file_content += dialogue.character + ": " + dialogue.dialogue + "\n"
				
			dialogue_file_content += "=> END\n"
			
			# Write new file
			var file = FileAccess.open("res://dialogues/coversations/sushil.dialogue", FileAccess.WRITE)
...
func trigger_dialogue_on_start() -> void:
	...
	balloon.start(load("res://dialogues/coversations/sushil.dialogue"), "start")
